...
interface Props {
  ...
  selectedMessage: MessageDetails;
    mode?: "reply" | "replyAll";  // ⭐ add this line

}

const RichTextEditorReply: React.FC<Props> = ({ onCancel, selectedMessage,mode = "reply" }) => {
  ...
  const handleSend = async () => {
    ...
    formData.append('in_reply_to', String(selectedMessage.id));
    formData.append('mode', mode);
    formData.append('to', JSON.stringify([
      { email: selectedMessage.from_email.email, name: selectedMessage.from_email.name || '' }
    ]));
    // Attachments
    ...
    // Here you can send formData using composeMessage(formData)
    ...
    <div className="border rounded-md p-4 space-y-4 bg-white">
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex items-center space-x-2">
          ...
          <input
            value={selectedMessage.from_email.email}
            disabled
